...
module.exports = {
  ...
  checkout: async (req, res) => {
    try {
      ...
      if (!res_nominal)
        return res.status(402).json({ message: "nominal tidak ditemukan." });

      const res_payment = await Payment.findOne({ _id: payment });

      if (!res_payment)
        return res.status(402).json({ message: "payment tidak ditemukan." });
  ...
  editProfile: async (req, res) => {
    try {
      ...
      if (req.file) {
        ...
        let filename = req.file.filename + "." + origninalExt;
